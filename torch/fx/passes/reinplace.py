...
def reinplace(gm, *sample_args):
    ...
    **(1)** Perform some initial checks on the metadata of "a" and "args..."
    ...
    - **(1c)** Check if "a" and "args..." alias.
      For example, re-inplacing to create ``aten.mul_(a, a)``
    ...
    **(3)** Perform the actual re-inplacing.

    - **(3a)** For ``{view}_scatter`` ops, replace with view + copy::
    ...
    **(4)** Update all later nodes that use "b" as an argument
    to take in "a" instead.

    **(5)** Any ``{view}_scatter`` nodes identified as safe to ignore
